...
router.get('/stats', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    const cotisationsAujourdhui = await pool.query(`SELECT COUNT(*) FROM cotisations WHERE DATE(date_cotisation) = CURRENT_DATE AND statut = 'valide'`);
    const totalCollecte = await pool.query(`SELECT COALESCE(SUM(montant), 0) as total FROM cotisations WHERE DATE(date_cotisation) = CURRENT_DATE AND statut = 'valide'`);
    res.json({
...
router.get('/cotisations/today', verifyToken, isAdmin, async (req, res) => {
  try {
    const result = await pool.query(`SELECT u.nom, u.prenom, c.montant, c.statut, c.heure_validation FROM cotisations c JOIN membres m ON m.id = c.membre_id JOIN users u ON u.id = m.user_id WHERE DATE(c.date_cotisation) = CURRENT_DATE ORDER BY c.created_at DESC`);
...
router.get('/export/cotisations', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT u.nom, u.prenom, c.montant, c.statut, c.mode_paiement, c.date_cotisation, c.heure_validation FROM cotisations c JOIN membres m ON m.id = c.membre_id JOIN users u ON u.id = m.user_id WHERE c.date_cotisation BETWEEN $1 AND $2 ORDER BY c.date_cotisation`, [debut, fin]);
    const csvWriter = createCsvWriter({ header: [{ id: 'nom', title: 'Nom' }, { id: 'prenom', title: 'Prénom' }, { id: 'montant', title: 'Montant' }, { id: 'statut', title: 'Statut' }, { id: 'mode_paiement', title: 'Mode de paiement' }, { id: 'date_cotisation', title: 'Date' }] });
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', 'attachment; filename=export_cotisations.csv');
    res.send(csvWriter.getHeaderString() + csvWriter.stringifyRecords(result.rows));
  } catch (err) { console.error(err); res.status(500).json({ message: 'Erreur serveur' }); }
});
...
router.post('/collecteurs', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    await pool.query(`INSERT INTO users (nom, prenom, email, mot_de_passe, role, microfinance_id) VALUES ($1, $2, $3, $4, 'collecteur', $5)`, [nom, prenom, email, hash, microfinanceId]);
...
router.post('/membres', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    const motDePasseTemp = Math.random().toString(36).slice(-8);
    const hash = await bcrypt.hash(motDePasseTemp, 10);
    const newUser = await pool.query(`INSERT INTO users (nom, prenom, email, mot_de_passe, role, microfinance_id) VALUES ($1, $2, $3, $4, 'membre', $5) RETURNING id`, [nom, prenom, email, hash, microfinanceId]);
    await pool.query(`INSERT INTO membres (user_id, solde, collecteur_id) VALUES ($1, 0, $2)`, [newUser.rows[0].id, collecteur_id]);
...
router.get('/collecteurs', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    const adminInfo = await pool.query('SELECT microfinance_id FROM users WHERE id = $1', [adminId]);
    const microfinanceId = adminInfo.rows[0].microfinance_id;
    const result = await pool.query(`SELECT id, nom, prenom FROM users WHERE role = 'collecteur' AND microfinance_id = $1 ORDER BY nom`, [microfinanceId]);
    res.json(result.rows);
  } catch (err) { console.error(err); res.status(500).json({ message: 'Erreur serveur' }); }
});
...
router.get('/retraits/en-attente', verifyToken, isAdmin, async (req, res) => {
  try {
    const result = await pool.query(`SELECT dr.id, dr.montant, dr.mode_paiement, dr.created_at, u.nom, u.prenom FROM demandes_retrait dr JOIN membres m ON m.id = dr.membre_id JOIN users u ON u.id = m.user_id WHERE dr.statut = 'en_attente' ORDER BY dr.created_at ASC`);
    res.json(result.rows);
  } catch (err) { console.error(err); res.status(500).json({ message: 'Erreur serveur' }); }
});
